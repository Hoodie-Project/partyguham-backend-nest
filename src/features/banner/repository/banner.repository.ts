...
export class BannerRepository {
  ...
  async findAllWebAndCount() {
    const result = await this.bannerRepository.findAndCount({ where: { platform: BannerPlatformEnum.Web } });

    return result;
  }

  async findAllAppAndCount() {
    const result = await this.bannerRepository.findAndCount({ where: { platform: BannerPlatformEnum.App } });

    return result;
  }

  async findAll() {
    const result = await this.bannerRepository.find();
  ...
  async findByIds(ids: number[]) {
    const result = await this.bannerRepository.find({ where: { id: In(ids) } });
