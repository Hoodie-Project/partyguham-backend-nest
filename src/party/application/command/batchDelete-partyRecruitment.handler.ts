...
import { BatchDeletePartyRecruitmentCommand } from './batchDelete-partyRecruitment.comand';

@Injectable()
@CommandHandler(BatchDeletePartyRecruitmentCommand)
export class BatchDeletePartyRecruitmentHandler implements ICommandHandler<BatchDeletePartyRecruitmentCommand> {
  constructor(
  ...
  async execute(command: BatchDeletePartyRecruitmentCommand) {
    ...
    const partyRecruitment = await this.partyRecruitmentRepository.batchDelete(partyRecruitmentIds);
  }
}
